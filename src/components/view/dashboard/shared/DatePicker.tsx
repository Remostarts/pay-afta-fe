  ...
  const renderDateRangeText = () => {
    ...
    return (
      <div className="flex justify-center gap-8 p-1">
        <div className="w-[180px] rounded-md border border-gray-300 py-2 text-center">
          {fromText}
        </div>
        <div className="flex items-center">-</div>
        <div className="w-[180px] rounded-md border border-gray-300 py-2 text-center">{toText}</div>
      </div>
    );
  };
        ...
        {weeks.map((week, weekIndex) => (
          <div key={weekIndex} className="z-10 grid grid-cols-7 bg-white">
            {week.map((day, dayIndex) => {
              ...
              let bgClass = '';
              if (isSelected) bgClass = 'bg-[#E6E7FE]';
              if (isStart) bgClass = 'bg-[#070AC5] rounded-full text-white';
              if (isEnd) bgClass = 'bg-[#070AC5] text-white rounded-full';
    ...
    <div className={cn('flex flex-col', className)}>
      ...
      <div className="flex gap-4 p-4">
        <Button
          ...
          variant="outline"
          className="flex-1 rounded-full border border-[#03045B]"
        >
          Cancel
        </Button>
        <Button onClick={handleApply} className="flex-1 rounded-full bg-[#03045B] text-white">
          Apply
        </Button>
      </div>
    </div>
