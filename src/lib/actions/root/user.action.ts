...
export const getUserTransactions = async (page: number = 1, limit: number = 10) => {
  const session = (await getServerSession(authOptions)) as any;
  const token = session?.accessToken;
  ...
  const response = await fetch(
    `${process.env.NEXT_PUBLIC_BACKEND_URL}/user/get-user-transactions?page=${page}&limit=${limit}`,
    {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
